...
def user_login():
    ...
    :return:
    """
    user_info = {"user_name": "test01", "passwd": "admin123"}
...
def add_user():
    ...
    if all([username, password, role_id, dates, phone]) and token == get_token["token"]:
        with open(
            DIR_BASE + "/data/mockdata/userManage.json", "a", encoding="utf-8"
        ) as f:
            add_user_info = {
...
def update_user():
    ...
    phone = flask.request.form.get("phone")
    if (
        username == update_user_info["username"]
        and password == update_user_info["password"]
        and role_id == update_user_info["role_id"]
        and dates == update_user_info["dates"]
        and phone == update_user_info["phone"]
    ):
...
def product_detail():
    ...
    response["goodsId"] = random.choice(
        ["18382788819", "33809635011", "56996760797", "82193785267", "74190550836"]
    )
...
def place_an_order():
    ...
    if all(
        ...
        if goods_id in good_id_list:
            ...
            write_data(
                DIR_BASE + "/data/mockdata/orderNumber.json",
                json.dumps({"order_num": order_num, "user_id": user_id}),
            )
            response = {
...
def assign():
    ...
    if cookie:
        ...
        if all([orderNo, orgId]):
            if orderNo == order_no.get("order_id") and orgId in [
                "4140913758110176843",
                "6140913758128971280",
            ]:
                response = {"code": 20000, "data": True, "message": "操作成功"}
    ...
    if cookie:
        ...
        if all([log_list, logisticsOrderId]):
            if logisticsOrderId == logistics_no.get("logistics_id"):
                if isinstance(log_list[0]["itemNum"], int) and isinstance(
                    log_list[0]["splitNum"], int
                ):
                    response = {
                        "code": 20000,
                        "message": "操作成功",
        ...
        if len(res_json_lst) > 0:
            for dicts in res_json_lst:
                for key, value in dicts.items():
                    if key == "logisticsOrderId" and value != logistics_no.get(
                        "logistics_id"
                    ):
            ...
            if orderNo == schedule_no.get("schedule_id"):
                if all(
                    [
                        ...
                        isinstance(scheduleMapStatus, int),
    ...
    if cookie:
        ...
        if all(
            ...
            if weightNo == weightNum.get("weightNo"):
                if all(
                    [
                        isinstance(exceptOther, float),
                        isinstance(spareNum1, float),
                        isinstance(productNet, float),
                    ]
                ):
                    refund_num = (float(productNet) / float(exceptOther)) * (
                        float(spareNum1) - float(exceptOther)
                    )
                    response = {
                        "code": 20000,
                        "message": "处理成功",
        ...
        if all([billName, dataType, costBillStatus, orderNo, ids]):
            if orderNo == schedule_no.get("schedule_id"):
                ...
                response = {
                    ...
                    "message": "操作成功",
                    "data": True,
                    "reconciliationNum": reconciliation_id,
                }
                return jsonify(response)
            else:
                return jsonify({"message": "调度单id不存在", "code": 90000})
...
@jwt_required(locations=["headers"])
@api.route(
    "/monitor/accidentInvestigation/insertAccidentInvestigation", methods=["post"]
)
...
if __name__ == "__main__":
    # debug=False，生产/测试环境关闭，错误返回 JSON 而非 HTML 调试页
    api.run(host="127.0.0.1", port=8787, debug=False)
